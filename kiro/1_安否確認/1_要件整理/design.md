...
- Lambda Triggers：PreAuthentication（LockoutTable 参照）、PostAuthentication（監査ログ）、PreSignUp（管理者作成のみ許可）。
...
- `vs ∈ {SAFE, INJURED, UNAVAILABLE}` ⇒ false（有効応答取得済、再発信不要）
- `vs ∈ {OTHER, PENDING}` または `cc ∈ {NO_ANSWER, BUSY, VOICEMAIL, ERROR, TRANSCRIBE_FAILED}` のいずれかであり、かつ `a < R` ⇒ true
- `a ≥ R` ⇒ false（試行上限到達、`UNREACHABLE` 確定）

有効 Voice_Status 受領後は以降の発信が起きないことが帰結する。
...
_For all_ Connect `DisconnectReason`、Transcribe ジョブステータス、CallEndHandler 入力について、`classifyCallResult(reason, transcribeStatus, recorded)` は以下のマッピングに完全に従う：
...
6. `summary.byStatus[s] ≥ 0` for all `s ∈ {SAFE, INJURED, UNAVAILABLE, OTHER, UNREACHABLE, PENDING}`

**Validates: Requirements 11.2, 11.3**

### Property 16: 完了判定

_For all_ Cycle に紐づく Response 集合 `Rs` について、`isCycleCompleted(Rs)` は `∀r ∈ Rs : r.voiceStatus ≠ PENDING` のときに限り true を返す。すなわち全対象者の最終ステータス確定が完了判定の必要十分条件である。
...
- 元々確定済の `r.voiceStatus`（`PENDING` 以外）は変更されない
- `r.voiceStatus = PENDING` であった全レコードは `r.voiceStatus = UNREACHABLE` に更新される
...
- いずれかの `q_i = Success(status)` で `status ∈ {COMPLETED, TIMEOUT}` 以降、ポーリング呼出は発行されない（発行回数の上限が `i` で停止）
...
_For all_ 監査対象イベント `ev`（認証成功 / 失敗 / 社員マスタ更新 / Cycle 起動 / 電話番号更新 / 辞書更新 / Inbound_Contact）について、`auditLogEntry(ev)` の出力文字列 `L` は以下のフィールドをすべて含む：(1) イベント種別、(2) UTC ISO 8601 形式のタイムスタンプ、(3) 実行者識別子（プリンシパル）、(4) 対象識別子（社員 ID または辞書キーワードまたは Cycle ID など）、(5) 電話番号を含む場合は Property 22 のマスキング適用後の値のみ。
...
| `LoadTargetsFn` / `ConnectDispatcherFn` / `CallEndHandlerFn` / `TranscribeStarterFn` / `KeywordMatcherFn` / `RetryEvaluatorFn` / `CycleFinalizerFn` / `RecordingMetadataWriterFn` / `InboundHandlerFn` | `AWS::Lambda::Function`                            | SFN / Connect / Transcribe / Inbound 処理                                        |
...
Rules:
  TokyoRegionOnly:
    Assertions:
      - Assert: !Equals [!Ref "AWS::Region", "ap-northeast-1"]
        AssertDescription: "This template must be deployed to ap-northeast-1 (Requirement 17.6)"
```

`Rules` セクションで非東京リージョンへのデプロイを CFn 検証段階で拒否する。
...
| 17.1〜17.6     | IaC                             | template.yaml + Parameters + Mappings + Rules(Region Assert)                                                         | P4                 |
...
| 別リージョン誤デプロイ                  | 要件違反                                          | CFn `Rules` セクションで `ap-northeast-1` 以外を Assertion 失敗で拒否。                                                                                                                                                             |

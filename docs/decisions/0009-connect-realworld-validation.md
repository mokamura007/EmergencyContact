...
2. **Outbound 用**：DID（直通番号）、Japan、**Local（050 IP 電話プレフィックス、2026-07-03 セッションでユーザー確定）**。dev のコスト最小化 + 迷惑電話フィルタ回避 + 書類提出リスク回避のため Toll Free は不採用
3. **Inbound 用**：DID（直通番号）、Japan、**Local（050 IP 電話プレフィックス、Outbound とは別番号、2026-07-03 セッションでユーザー確定）**。dev のコスト最小化のため Toll Free は不採用（prod 移行時に再検討候補、コード変更ゼロで番号 ARN 差し替えのみで切替可能）
4. 取得した 2 本それぞれの ARN + Outbound は E.164 形式の電話番号本体も控える：
   - `ConnectOutboundPhoneNumberArn`
   - `ConnectOutboundPhoneNumber`（Outbound 番号の E.164 表現、Phase 6.2 ConnectDispatcher が `SourcePhoneNumber` に使用、template.yaml L44 定義）
...
2. Step 1〜3 で控えた 7 項目を投入：
   ```json
   {
     ...
     "ConnectOutboundPhoneNumber": "<Step 2 で控えた Outbound 番号の E.164 表現、例：+815012345678>",
